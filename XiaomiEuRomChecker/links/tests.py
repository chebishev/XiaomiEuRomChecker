...
class Test(TestCase):
    ...
    def test_invalid_url_shortener_with_integer(self):
        ...
        with self.assertRaises(AttributeError):
            self.assertEqual(shorten_url(first_url), "'int' object has no attribute 'startswith'")

    def test_invalid_url_shortener_with_string(self):
        first_url = "atanas"
        with self.assertRaises(ShorteningErrorException):
            self.assertEqual(shorten_url(first_url), 'There was an error on trying to short the url: b\'{'
