...
def theme_colors(request):
    ...
    with open("XiaomiEuRomChecker/theme/json/custom_theme.json", "r", encoding="utf-8") as theme_file:
        colors = json.load(theme_file)
    return {'color': colors}


def tip_of_the_day(request):
    with open("XiaomiEuRomChecker/theme/json/tips.json", "r", encoding="utf-8") as tips_file:
